...
const UserTableRow = ({ 
  ...
  return (
    <View style={styles.tableRow}>
      <Text style={[styles.tableCell, styles.idCell]}>{item.registrationId}</Text>
      <Text style={[styles.tableCell, styles.nameCell]}>{item.fullName}</Text>
      <Text style={[styles.tableCell, styles.emailCell]}>{item.emailAddress}</Text>
          ...
          <View style={styles.acceptedContainer}>
            ...
            {item.userName && item.password && (
              <TouchableOpacity 
                ...
                onPress={() => onResendCredentials(
                  item.registrationId,
                  item.emailAddress, 
                  Array.isArray(item.userName) ? item.userName[0] : item.userName,
                  Array.isArray(item.password) ? item.password[0] : item.password
                )}
              >
