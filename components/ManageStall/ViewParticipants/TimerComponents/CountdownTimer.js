...
const CountdownTimer = ({ endTime, isRunning, onTimerEnd }) => {
  ...
  useEffect(() => {
    ...
    // Clear any existing timer state if not running or no endTime
    if (!endTime || !isRunning || endTime === null || endTime === 'null') {
      console.log('Timer not running - endTime:', endTime, 'isRunning:', isRunning);
      setTimeLeft(null);
      setIsExpired(false);
      return;
    }
    ...
    const calculateTimeLeft = () => {
      ...
      if (typeof endTime === 'string') {
        ...
        // Ensure we have milliseconds, not seconds
        if (endTime < 10000000000) { // If less than year 2001 in milliseconds, it's probably seconds
    ...
    // Update every second only if we have valid endTime
        ...
        {timeLeft.days > 0 && (
          <>
            <View style={styles.timeUnit}>
              ...
              <Text style={styles.timeLabel}>Day{timeLeft.days !== 1 ? 's' : ''}</Text>
            </View>
