  ...
  if (!isInitialized) {
    try {
      emailjs.init({
        ...
        blockHeadless: false, // Changed to false for development
        blockList: {
          list: [],
        ...
        limitRate: {
          id: 'app',
          throttle: 5000, // Reduced throttle time
        },
      });
      isInitialized = true;
...
// Method 1: Using fetch with proper headers (bypasses EmailJS CORS issues)
export const sendCredentialsEmailFetch = async (recipientEmail, username, password) => {
  try {
...
// Method 2: Using XMLHttpRequest (alternative to fetch)
export const sendCredentialsEmailXHR = async (recipientEmail, username, password) => {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            resolve({ success: true, message: `Credentials sent to ${recipientEmail}` });
          } else {
...
// Method 3: Improved EmailJS with better error handling
export const sendCredentialsEmailImproved = async (recipientEmail, username, password) => {
  initializeEmailJS();
  
  try {
    console.log('📧 Sending email via improved EmailJS method to:', recipientEmail);
    
    // Wait a bit to ensure EmailJS is fully initialized
...
// Enhanced retry mechanism with multiple methods
export const sendCredentialsEmailWithRetry = async (recipientEmail, username, password) => {
  console.log('📧 Starting enhanced email retry mechanism');
  
  // Method 1: Try fetch method first (most reliable for CORS)
  console.log('📧 Attempt 1: Fetch method');
  let result = await sendCredentialsEmailFetch(recipientEmail, username, password);
  if (result.success) {
    return result;
  }
  
  // Method 2: Try XHR method
  ...
  // Method 3: Try improved EmailJS method
  console.log('📧 Attempt 3: Improved EmailJS method');
  result = await sendCredentialsEmailImproved(recipientEmail, username, password);
  if (result.success) {
    return result;
  }
  ...
  return {
    success: false,
    message: 'All email sending methods failed. Please check your network connection and try again.'
  };
};
...
Admin Team`;
    ...
    // Use the fetch method for status notifications too
    const templateParams = {
      service_id: EMAILJS_SERVICE_ID,
      template_id: EMAILJS_TEMPLATE_ID,
      user_id: EMAILJS_PUBLIC_KEY,
...
// Development CORS bypass function (for testing only)
export const createCORSProxy = () => {
  // This is a development helper - you can use a CORS proxy service
...
Admin Team`,
        ...
        const response = await fetch(CORS_PROXY + 'https://api.emailjs.com/api/v1.0/email/send', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
